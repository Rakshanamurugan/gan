...
import nibabel as nib  # Library to handle NIfTI files
import psutil
...
def save_subplot_image(reconstructed, real, psnr_value, output_path):
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))

    axes[0].imshow(reconstructed, cmap='gray')
    axes[0].set_title(f'Reconstructed Image\nPSNR: {psnr_value:.2f}')

    axes[1].imshow(real, cmap='gray')
...
if __name__ == '__main__':
    ...
    for i, data in enumerate(dataset):
        ...
        patient_images[patient_id].append((reconstructed_img, real_img, psnr_value, filename))
    ...
    for patient_id, patient_data in patient_images.items():
        ...
        for reconstructed_img, real_img, psnr_value, original_filename in patient_data:
            save_subplot_image(reconstructed_img, real_img, psnr_value, os.path.join(patient_dir, original_filename))
